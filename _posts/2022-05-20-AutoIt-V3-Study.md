...
WinWaitActive함수는 특정 대화상자가 화면에 활성화되기를 기다리는 함수입니다. "제목 없음 - Windows 메모장"이란 제목을 가진 대화상자가 나타날 때까지 오토잇은 대기상태에 들어 갑니다. 즉 메모장이 실행되기를 기다리는 거지요. 메모장 프로그램이 활성화 되면 다음 줄이 실행됩니다.
...
오토잇에서 변수는 어떤 데이터에도 자유롭게 붙일 수 있습니다. 같은 변수를 다른 종류의 데이터에 붙여도 괜찮습니다. 이것을 동적 타이핑(dynamic typing)이라 부릅니다. 동적 타이핑 방식의 언어들은 초보자들 입장에서 코딩하기에는 편리하지만 프로그램의 연산 속도를 떨어 뜨리는 주범이기도 합니다. 대다수의 스크립트 언어들이 컴파일 언어들에 비해 속도가 현저하게 늦은 이유가 바로 이것입니다.
...
Not는 참을 오류로 바꾸고 오류를 참으로 바꾸는 역할을 합니다.
...
변수 이름을 지을 때에도 규칙을 정해서 사용하면 좋습니다. 오토잇은 "헝가리언 표기법"을 따른다고 합니다. 먼저 $ 다음에 데이터 타입을 알 수 있는 소문자로 시작하고 그 다음에 실질적인 변수의 이름을 적습니다. 데이터 타입과 예제는 다음과 같습니다.
...
변수 뒤에는 변수의 이름을 적습니다. 만약 사람 이름이라면 $sName라는 식입니다. 이때 Name의 첫 글자는 대문자로 사용합니다. 함수명을 적을 때와 같은 규칙을 사용하는 게 좋습니다.
...
변수를 초기화할 때 Local, Global, Dim 키워드를 생략할 수도 있습니다만, 추천하지는 않습니다. 명시적으로 Local, Global 키워드를 사용하세요.
...
배열에 저장될 데이터의 숫자는 정해져 있습니다. 이것을 배열의 크기라 불러도 좋습니다. 배열의 크기를 조절하려면 Redim을 사용하세요.
...
논리 연산자도 있습니다. And, Or, Not가 대표적입니다.
...
프로그래밍 언어마다 조건문과 유사하게 이른바 삼항 연산자를 지원합니다.  오토잇도 그렇습니다. "?" 앞에 조건을 넣고 True일 때, False일 때 할 행동은 ":"으로 표현합니다. 예를 들어 보겠습니다. 다음 코드를 입력하고 이름을 if.au3으로 저장하세요.

; 07_if.au3

$Money = InputBox("남은 용돈", "금액")
If $Money > 50000 Then
...
If 대신 Select, Switch 구문으로 표현할 수 있습니다. 다음 코드를 입력하고 파일이름을 select.au3으로 저장하세요.
...
파이썬(Python) 언어의 경우 조건문을 오로지 if로만 작성합니다. 그렇게 해도 문제가 없기 때문입니다. 여러분도 Select와 Switch는 대략 이런 것이 있구나 하고 지나가세요. 대신 If는 열심히 공부하세요.
...
숫자의 경우 0은 False이고 나머지는 True입니다. 따라서 While 1을 하면 무조건 무한 반복이 실행됩니다. 무한 반복을 종료시키려면 어떻게 해야 할까요? 그럴 때에는 ExitLoop 키워드를 사용하세요.  아래 코드를 입력하고 while.au3이라고 저장하세요.
...
반복을 강제로 종료할 때 ExitLoop를 쓴다면 불필요한 코드를 반복하지 않고 건너 뛰게 하는 ContinueLoop 키워드도 있습니다. 아래 코드를 입력하고 continue.au3으로 저장하세요.
; 11_continue.au3
...
무조건 딱 한번 실행한 다음에 While과 유사하게 동작시키는 반복도 있습니다. 바로 Do ~ Until 입니다. 아래 코드를 입력하고 until.au3으로 저장하세요.
...
오토잇이 실행될 때 Do 키워드를 만나면 아래 코드를 1회 무조건 실행합니다. Until 키워드 뒤에는 조건식이 있는데 이 조건을 만족하면 반복 실행을 멈춥니다. 조건식이 False가 될 때 다시 Do 키워드로 올라가서 다시 코드를 반반복 실행합니다.
...
함수는 Func와 EndFunc로 정의합니다. 함수이름은 영어로 시작해야 합니다. 매개변수는 사용해도 되고 안 해도 됩니다. 

다음 코드를 입력하고 hotkey.au3으로 저장한 후 실행해 보세요.
...
만약 While로 대기하지 않았다면 이 프로그램은 종료되고 말았을 겁니다. ALT+f를 눌러도 아무런 일도 일어나지 않았을 겁니다. 단축키가 눌릴 때까지 대기해야 한데 이럴 때 While 1을 사용합니다.
Sleep 함수는 코드 실행을 일시정지시킬 때 사용합니다. Sleep을 넣지 않아도 상관 없습니다만 불필요한 낭비를 줄이기 위해 Sleep을 사용했습니다.
...
오토잇에는 메시지루프모드, 이벤트모드 두 가지로 GUI를 구현할 수 있습니다. 간단한 프로그램은 메시지루프를 사요하시고 복잡한 프로그램은 이벤트모드를 사용하세요.

20. 대화상자 만들기

대화상자를 만드려면 GUICreate 함수와 GUISetState 함수를 사용합니다.
...
대화상자에 첫번째로 들어갈 부품은 버튼입니다. 버튼을 누르면 프로그램이 종료되는 코드를 만들어 보겠습니다.
...
GUICtrlCreateButton은 버튼을 만드는 함수입니다. 윈도우핸들을 지정하지 않으면 가장 최근에 만들어진 윈도우에 버튼이 들어 갑니다.
...
위 예제는 메시지루프 방식으로 구현된 GUI입니다. 

GUIGetMsg 함수는 오토잇에서 만든 윈도우를 감시하며 어떤 부품이 동작했는지를 알려줍니다. 만약 버튼을 눌렀다면 $idButton이 반환됩니다. 버튼을 누르면 메시지상자가 나타납니다. 

MsgBox의 네번째 매개변수는 지연시간입니다. 단위는 초이며 0이면 무한 대기, 5이면 5초 후에 자동으로 메시지상자가 사라집니다.
다섯번째 매개변수는 부모윈도우의 핸들입니다. 윈도우의 각 부품들은 자신의 소속을 명시적으로 표시할 수 있습니다. MsgBox 함수에서 마지막 매개변수를 삭제하고 실행해 보세요. 그리고 두 경우의 차이점이 무엇인지 살펴 보세요.

22. 라벨과 한줄 편집창

PC용 카카오톡 같은 프로그램을 실행하면 최초에 로그인 대화상자가 나타납니다. 우리도 대화상자를 만들어 봅시다. 먼저 우리가 사용할 함수는 편집창을 만드는 기능을 해야 합니다.

GUICtrlCreateInput 함수는 1줄 짜리 편집창을 만듭니다. 아이디, 비밀번호는 한줄 편집창이면 충분할 겁니다.

그런데 이 편집창이 무엇을 입력해야 하는지 알 방법이 없습니다. 그래서 편집창 앞에 라벨을 하나 붙입니다. '아이디'라고 말이죠. 라벨은 
GUICtrlCreateLabel 함수로 만들 수 있습니다.

자! 이 지난 시간에 배운 버튼과 이번 시간에 배운 것을 활용하면 로그인 대화상자를 만들 수 있습니다. 
...
Switch GUIGetMsg()
Case $idButtonCancel
MsgBox(0, "알림", "로그인을 취소합니다. 프로그램을 종료합니다.", 0, $hWin)
ExitLoop
EndSwitch
WEnd

#Include는 이미 만들어진 au3 파일을 불러오는 명령입니다. EditConstants.au3 파일에는 편집창과 관련된 상수들이 정의되어 있습니다. 로그인을 할 때 비밀번호도 입력해야 하는데요. 이때 비밀번호가 노출될 수 있습니다. 그래서 비밀번호를 입력할 때에는 '*'가 편집창에 들어가게 됩니다. 이 기능을 위해 EditConstants.au3 파일을 불러 들였습니다. GUI 관련된 상수는 매우 많습니다. 이것들을 모두 기억할 수 없기 때문에 EditConstants.au3 같은 파일이 필요한 셈이죠.

편집창 앞에 라벨을 넣으면 스크린리더는 편집창이 포커스 될 때마다 바로 앞에 있는 라벨을 읽어 줍니다.

GUICtrlCreateInput 함수의 첫번째 매개변수는 문자열인데, 편집창 안에 들어갈 텍스트입니다. 다여섯번째 매개변수는 편집창의 스타일을 지정합니다. $ES_PASSWaORD는 비밀번호 입력 스타일입니다.

현재 이 코드는 아무런 동작을 하지 않습니다. 취소 버튼을 눌렀을 때만 동작합니다. 메시지루프 방식이 간단하긴 하지만 로그인 동작 같은 것을 시킬 때에는 오히려 코드가 복잡해집니다. 그래서 온이벤트모드(OnEventMode)를 배울 필요가 있습니다.

23. 온이벤트모드

온이벤트모드는 메시지루프와 코딩 스타일이 다릅니다. 핵심은 윈도우에 들어간 부품들이 동작할 때 그 동작과 함수를 하나로 묶는 것에 있습니다. 일단 아래 코드를 봅시다.
...
Opt 함수는 오토잇의 환경설정을 할 수 있습니다. 만약 GUI를 온이벤트모드로 만들고 싶다면 아래와 같이 쓰세요.
...
메시지루프에서는 While 사이에 GUIGetMsg 함수를 넣었습니다. 반면 온이벤트모드에서는 While에는 Sleep(100)이 들어 있습니다. Sleep를 넣지 않으면 오토잇이 불필요하게 CPU를 점유합니다.
...
지금까지 알아본 것들을 기초로 해서 주소록을 한번 만들어 보겠습니다. 주소록에는 사람 이름, 전화번호, 주소, 메모 네 가지가 기록되어야 합니다. 전체 주소록 목록이 나타날 수도 있고 일부 목록만 나타날 수도 있습니다. 검색 기능도 가진 것이지요.
주소록에 입력된 내용은 파일에 저장됩니다. 이때 INI 파일을 이용하겠습니다.

화면에는 주소록이 표시될 목록상자가 있어야 합니다. 주소록에서 팝업키를 누르면 주소 추가, 수정, 삭제, 검색, 변경 사항 저장, 종료 메뉴가 나타나야 합니다. 물론 이 메뉴들은 단축키로도 실행 가능해야 합니다.

추가나 수정을 누르면 주소록을 입력할 별도의 대화상자가 열려야 합니다.

삭제를 누르면 정말로 삭제할 것인지 물어보는 메시지상자를 표시합니다.

검색을 누르면 검색 종류 콤보상자, 검색창, 검색 버튼이 들어 있는 대화상자가 표시되어야 합니다.

종료를 누르면 변경 사항이 있을 경우 변경 사항을 저장할 것인지를 물어 보고 종료되어야 합니다.

초보자에게는 어려워 보일 겁니다만 천천히 하나씩 만들어 가면 누구나 쉽게 할 수 있는 작업들입니다.

27. 목록상자 만들기

목록상자에는 두 종류가 있습니다. ListBox와 ListView입니다. 우리가 사용할 목록상자는 이름, 전화번호 등 여러 칼럼이 필요하므로 ListView를 사용합니다. ListView를 만드려면 GUICtrlCreateListView 함수를 사용합니다. 이 함수의 첫번째 매개변수는 칼럼이름입니다. 칼럼 이름은 버티컬바(|)로 구분하세요. 
...
Global $hMain = GUICreate("주소록", 800, 600)
GUISetOnEvent($GUI_EVENT_CLOSE, "OnMainClose")

Global $idListView = GUICtrlCreateListView("이름|전화번호|주소|메모", 10, 10, 780, 580)

$idFileMenu = GUICtrlCreateMenu("파일(&F)")
$idSaveMenuItem = GUICtrlCreateMenuItem("저장(&S)", $idFileMenu)
$idExitMenuItem = GUICtrlCreateMenuItem("종료(&X)", $idFileMenu)
GUICtrlSetOnEvent($idExitMenuItem, "OnMainClose")

$idEditMenu = GUICtrlCreateMenu("편집(&E)")
$idAppendMenuItem = GUICtrlCreateMenuItem("추가(&A)", $idEditMenu)
GUICtrlSetOnEvent($idAppendMenuItem, "OnAppendMenuItem")
...
Func OnAppendMenuItem()
$s = InputBox("추가", "입력", "|||")
$idLVItem = GUICtrlCreateListViewItem($s, $idListView)
EndFunc

위 소스 코드에서는 GUICtrlCreateListView  함수로 ListView를 만들었씁니다. 이 ListView는 네 개의 칼럼을 갖습니다. 각 칼럼의 머리말은 이름, 전화, 주소, 메모입니다. 이 함수의 첫번째 매개변수가 바로 이것을 의미합니다.

ListView에 들어가는 항목은 GUICtrlCreateListViewItem 함수로 만듭니다. 

첫 번째 매개변수는 항목의 내용입니다. 칼럼 수에 맞춰서 버티컬바(|)로 내용을 구분해 주세요. 

두번째 매개변수는 이 리스트뷰아이템이 들어갈 ListView를 적어 줍니다.

28. 추가 대화상자를 그럴 듯하게

이전 소스에서 우리는 새로운 리스트뷰아이템을 InputBox를 이용해서 받았습니다. 그런데 뭔가 허술해 보입니다. 사용자가 알아서 버티컬바를 써 줘야 하는 불편도 있습니다. 그래서 이번에는 추가 대화상자를 그럴 듯하게 만들어 보겠습니다.

우선 GUICreate는 가능하면 한 파일에 한번씩 써 주시는 게 좋습니다. GUI가 한 페이지에 여러 개 들어가면 윈도우/컨트롤과 이벤트 처리 함수들을 혼동하기 쉽습니다. 그래서 추가 대화상자는 다른 파일에 작성하겠습니다. 그리고 #Include를 사용하여 메인 프로그램에 포함시키겠습니다. 소스의 상단에 다음을 추가해 주세요.

#Include"22-2_EditDialog.au3"

#Include 뒤에 "<"으로 시작하면 오토잇 include 폴더에 들어 있는 파일들이 메인 프로그램에 추가 됩니다. 예를 들어 GUIConstantsEx.au3 파일의 내용이 메인 프로그램에 포함되는 것처럼 말입니다.

또, 이번에 만들 대화상자는 새로운 항목을 추가할 뿐만 아니라 기존 내용을 수정할 수도 있어야 합니다. 