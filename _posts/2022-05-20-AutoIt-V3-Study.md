...
MP3DirectCut을 실행합니다. 실행된 프로세스 아이디는 $pMP3DC에 저장됩니다.
...
GUI는 일반적인 윈도우 프로그램을 말합니다. GUI는 'graphical user interface'의 첫 글자를 딴 것입니다줄여서 "구이"라고 부르기도 합니다. . 마이크로소프트 윈도우즈가 출시되기 전까지 도스(DOS)라는 운영체제를 사용했습니다. 검정색 바탕에 하얀 글자만으로 되어 있었습니다. 텍스트 위주의 운영체제와 대비하는 측면에서 GUI라는 말이 생겨났습니다. 오토잇은 비교적 GUI를 쉽게 구현할 수 있습니다. 시각장애인이 손쉽게 구현할 수 있지만 단점도 있어요. 반응속도가 상대적으로 느리다는 겁니다.

우리는 당분간 GUI를 배울 예정입니다. 그 이후 두 번째 과제를 해결해 보겠습니다. 두 번째 과제는 데이지 변환기입니다. GUI를 배워서 그럴 듯한 프로그램으로 완성시켜 봅시다.

오토잇에서는 대화상자를 윈도우라 부르고 윈도우 내에 배치되는 것들을 컨트롤이라 부릅니다. 윈도우는 핸들(handle)로 제어하고 컨트롤은 아이디(id)로 제어합니다. 
...
대화상자에 첫번째로 들어갈 부품은 버튼입니다. 버튼을 누르면 프로그램이 종료되는 코드를 만들어 보겠습니다. 이 예제는 메시지루프 방식으로 되어 있습니다. 

### 20_button.au3

$hWin = GUICreate("종료 버튼을 누르면 프로그램이 종료됩니다.", 400, 100)
$idButton = GUICtrlCreateButton("종료", 20, 20, 200, 40)
GUISetState(@SW_SHOW)

While 1
Switch GUIGetMsg()
Case $idButton
If MsgBox(1, "알림", "프로그램을 종료합니다.", 0, $hWin) == 1 Then Exit
EndSwitch
WEnd
...
첫번째 매개변수는 버튼에 표시될 텍스트입니다. 두번째부터 순서대로 윈도우 왼쪽 끝까지 거리, 윈도우 위쪽 끝까지 거리, 버튼의 가로 길이, 버튼의 세로 길이입니다.
...
스크린리더가 음성 지원을 비교적 잘 해주는 컨트롤들은 대략 다음과 같습니다.

- GUICtrlCreateButton: 버튼을 만듭니다.
- GUICtrlCreateCheckbox: 체크상자를 만듭니다.
- GUICtrlCreateCombo: 콤보상자를 만듭니다.
- GUICtrlCreateContextMenu: 팝업메뉴를 만듭니다.
- Creates a context menu for a control or entire GUI window.
- GUICtrlCreateDummy: 더미 컨트롤을 만듭니다.
- GUICtrlCreateEdit: 멀티라인 편집창을 만듭니다.
- GUICtrlCreateInput: 싱글라인 편집창을 만듭니다.
- GUICtrlCreateLabel: 고정 텍스트를 만듭니다.
- GUICtrlCreateList: 단순 목록상자를 만듭니다.
- GUICtrlCreateListView: 멀티칼럼 목록상자를 만듭니다.
- GUICtrlCreateMenu: 메뉴바를 만들거나 하위메뉴를 포함하는 메뉴를 만듭니다.
- GUICtrlCreateProgress: 진행율 표시줄을 만듭니다.
- GUICtrlCreateRadio: 라디오버튼을 만듭니다.
- GUICtrlCreateTab: 탭 컨트롤을 만듭니다.
- GUICtrlCreateTreeView: 트리뷰를 만듭니다.



## 24. 메시지루프

메시지루프 방식으로 동작하는 GUI 프로그램은 GUIGetMsg 함수를 사용해서 윈도우 안에서 어떤 일이 발생했는지를 체크합니다. WINAPI 방식이 바로 이 메시지루프입니다. 메시지루프 형식은 아주 간단한 GUI이거나 복잡도가 높은 GUI 모두 좋은 선택이 될 수 있습니다.메시지루프와 다른 형식인 온이벤트모드는 너무 복잡하지도 않고 너무 간단하지도 않은 중간 정도의 GUI에 적합합니다.

위 예제에서 만약 버튼을 눌렀다면 $idButton이 반환됩니다. 버튼을 누르면 메시지상자가 나타납니다. 
...
라벨(lavbel)은 고정 텍스트를 대화상자에 추가하는 컨트롤입니다. 고정텍스트는 단순히 정보를 알리기 위한 목적이기 때문에 TAB키를 눌러도 포커스되지 않습니다. 화면 탐색 혹은 객체 탐색으로 접근할 수 있습니다.

라벨은 고정텍스트(StaticText)라고도 하고 GUICtrlCreateLabel 함수로 만듭니다. 고정텍스트는 특별한 일을 하지 않으므로 굳이 반환값을 변수에 저장할 필요는 없습니다. 하지만 텍스트 내용을 변경시킬 예정이라면 라벨 컨트롤의 아이디를 반드시 변수에 저장하세요.
...
예제는 지난 시간에 배운 버튼도 활용한 계산기 프로그램입니다.
...
Case $idButtonExit, $GUI_EVENT_CLOSE
If MsgBox(1, "알림", "계산기를 정말로 종료할까요?", 0, $hWin) == 1 Then ExitLoop
EndSwitch
WEnd
...
GUIGetMsg 함수로 알아낸 값에는 윈도우 컨트롤들의 ID 값이 있습니다. 물론 대화상자의 이벤트도 감시합니다. 만약 GUIGetMsg가 알아낸 반환값이 계산버튼이라면 Case $idButtonOk 아래에 있는 코드가 실행됩니다. 만약 종료 버튼을 누른다면 ExitLoop 키워드가 동작하면서 While 반복문이 종료될 것이고 그 이하에 아무런 코드가 없기 때문이 이 프로그램 자체가 종료될 것입니다.
...
Func OnClose()
If MsgBox(1, "알림", "프로그램을 정말로 종료할까요?", 0, $hWin) == 1 Then Exit
EndFunc
...
메시지루프에서는 While 사이에 GUIGetMsg 함수를 넣었습니다. 반면 온이벤트모드에서는 While에는 Sleep(100)이 들어 있습니다. Sleep을 넣지 않으면 오토잇이 불필요하게 CPU를 점유합니다. 메시지루프 모드에서는 Sleep을 넣지 않아도 됩니다. 오토잇이 적절히 조절해 줍니다.
...
메뉴바는 시각장애인들이 풀다운 메뉴라 부르는 것입니다. 메뉴바를 추가하려면 GUICtrlCreateMenu 함수를 사용합니다.
...
이 정도면 GUI의 기초는 설명했다고 할 수 있습니다. 이제 두번째 도전 과제를 해결할 차례입니다. 데이지 변환기를 멋지게 만들어 봅시다.
